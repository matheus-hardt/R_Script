...
group <- 'THE GROUP YOU WHANT TO FETCH'
group_id <- searchGroup(group, token)
db <- getGroup(group_id[1,3], token, n=100)
#db <- getGroup(group_id[1,3], token, n=50000, since = "2008-01-01", until = "2008-12-31")
db$level <- 1
...
ids <- db_niv2$id
for (i in ids) {
  post <- getPost(i, token, n = 100000, likes = FALSE, comments = TRUE)
  db <- rbind.fill(db, post[["comments"]])
}
...
ids <- db_niv3$id
for (i in ids) {
  reply <- getCommentReplies(i, token, n = 1000, likes = FALSE, comments = TRUE)
  db <- rbind.fill(db, reply[["comments"]])
}
